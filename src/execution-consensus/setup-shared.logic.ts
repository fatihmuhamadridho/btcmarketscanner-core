import type { SupportResistance, SetupPathStep } from './types';
...
export function buildTakeProfitSteps(direction: 'long' | 'short', entryMid: number, risk: number, supportResistance: SupportResistance, targetBuffer: number) {
  if (direction === 'long') {
    return [
      { label: 'TP1' as const, price: Math.max(entryMid + risk, supportResistance.resistance + targetBuffer * 0.25) },
      { label: 'TP2' as const, price: Math.max(entryMid + risk * 2, supportResistance.resistance + targetBuffer * 0.75) },
      { label: 'TP3' as const, price: Math.max(entryMid + risk * 3, supportResistance.resistance + targetBuffer * 1.5) },
    ];
  }
  return [
